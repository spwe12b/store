...
import com.me.store.vo.OrderItemVo;
import com.me.store.vo.OrderProductVo;
import com.me.store.vo.OrderVo;
import com.me.store.vo.ShippingVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse<String> cancel(Integer userId,Long orderNo){
        ...
        int resultCount=orderMapper.insertSelective(updateOrder);
        if(resultCount>0){
    ...
    public ServerResponse getOrderCartProduct(Integer userId){
        ...
        orderProductVo.setImageHost(PropertiesUtil.getProperty(Const.IMGPREFIX));
    ...
    private OrderVo assembleOrderVo(Order order,List<OrderItem> orderItemList){
        ...
        orderVo.setImageHost(PropertiesUtil.getProperty(Const.IMGPREFIX));
        List<OrderItemVo> orderItemVoList=Lists.newArrayList();
        for(OrderItem orderItem:orderItemList){
     ...
     * @param orderNo  订单号
     * @param userId   用户ID
     * @param path     支付二维码的保存路径
     * @return
     */
    public ServerResponse pay(Long orderNo, Integer userId, String path) {
        Order order = orderMapper.selectByUserIdOrderNo(userId, orderNo);
        if (order == null) {
            return ServerResponse.createByErrorMessage("该用户没有当前订单");
        }
        Map<String, String> resultMap = Maps.newHashMap();
        resultMap.put(Const.ORDER_NO, orderNo.toString());
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        String outTradeNo = order.getOrderNo().toString();
        // (必填) 订单标题，粗略描述用户的支付目的。如“xxx品牌xxx门店当面付扫码消费”
        String subject = new StringBuilder().append("ps商场扫码支付").append(order.getOrderNo()).toString();
        ...
        String body = new StringBuilder().append("订单: ").append(order.getOrderNo()).append("购买商品共: ")
                .append(order.getPayment()).toString();
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                resultMap.put(Const.QR_URL,qrUrl);
    ...
    public ServerResponse alipayCallback(Map<String,String> params){
        Long orderNo=Long.parseLong(params.get(Const.AliCallback.OUT_TRADE_NO));
        String tradeNo=params.get(Const.AliCallback.TRADE_NO);
    ...
    public void closeOrder(int hour){
        ...
        List<Order> orderList=orderMapper.selectOrderByStatusCreateTime(Const.OrderStatus.NO_PAY.getCode()
                ,DateTimeUtil.dateToStr(closeDateTime));
        for(Order order:orderList){
            List<OrderItem> orderItemList=orderItemMapper.selectByOrderNo(order.getOrderNo());
            for(OrderItem orderItem:orderItemList){
             Integer stock=productMapper.selectStockByProductId(orderItem.getProductId());
             //订单里的商品有可能被删除，mybatis则返回空
             if(stock==null){
                 continue;
             }
